  ...
  return (
    <div className="space-y-6">
      <motion.div
        ...
        className="bg-surface rounded-2xl shadow-card border border-neutral-200 p-6 md:p-8"
      >
        <div className="text-center py-16">
          <div className="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-r from-accent-100 to-accent-200 rounded-full mb-6">
            <TrendingUp className="w-10 h-10 text-accent-600" />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-2xl mx-auto"></div>
        </div>
      </motion.div>
    </div>
  );
...
const BarcodeScanner = ({ isOpen, onClose, onScan }) => {
  ...
  // Additional check to ensure browser APIs are fully loaded
  useEffect(() => {
    if (isBrowser && isOpen) {
      // Add a small delay to ensure all browser APIs are ready
      const timer = setTimeout(() => {
        if (
          ...
          // Browser is ready, scanner can be initialized
